...
export default function MapViewScreen() {
  ...
  // Update nearby restaurants when map pans
  useEffect(() => {
    if (!filtered.length) return
    const sorted = [...filtered].sort((a, b) => {
      const da = Math.sqrt(Math.pow(a.latitude - mapCenter.lat, 2) + Math.pow(a.longitude - mapCenter.lng, 2))
      const db = Math.sqrt(Math.pow(b.latitude - mapCenter.lat, 2) + Math.pow(b.longitude - mapCenter.lng, 2))
      return da - db
    })
    // PlatePost customers always first
    const pro = sorted.filter(r => PLATEPOST_IDS.has(r.id))
    const rest = sorted.filter(r => !PLATEPOST_IDS.has(r.id))
    setNearbyRestaurants([...pro, ...rest].slice(0, 20))
  }, [mapCenter, activeFilter, restaurants.length, mapSearch])
                ...
                >
                  <motion.div
                    ...
                    style={{
                      ...
                      border: `3px solid ${isPro ? '#fff' : isCluster ? '#fff' : isSelected ? '#0048f9' : isFav ? '#fff' : '#e5e7eb'}`,
                      boxShadow: isPro || isSelected || isCluster
                        ? '0 4px 16px rgba(0,72,249,0.4)' : '0 2px 8px rgba(0,0,0,0.15)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      cursor: 'pointer',
                      transform: 'translate(-50%, -50%)',
                      zIndex: isPro ? 10 : isSelected ? 9 : isCluster ? 5 : 1,
                      transition: 'all 0.2s',
                    }}
                  >
                    {isCluster ? (
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#fff', fontFamily: 'Open Sans' }}>
                        {cluster.count}
                      </span>
                    ) : isPro ? (
                      <img src="/pp-mark.png" alt="" width={20} height={20}
                        style={{ objectFit: 'contain', filter: 'brightness(0) invert(1)' }} />
                    ) : (
                      <span style={{ fontSize: isSelected ? 20 : 16 }}>{emoji}</span>
                    )}
                  </motion.div>
                </OverlayView>
              )
      ...
      <div style={{
        ...
        <div ref={trayRef}
          ...
          {nearbyRestaurants.length === 0 ? (
            <p style={{ fontFamily: 'Open Sans', color: '#9ca3af', fontSize: 13 }}>No places found</p>
          ) : (
              ...
              return (
                ...
                >
                  <div style={{ position: 'relative', height: 80 }}>
                    ...
                    {isPro && (
                      <div style={{
                        ...
                        background: '#0048f9', borderRadius: 6, padding: '2px 6px',
                        display: 'flex', alignItems: 'center', gap: 3,
                      }}>
                        <img src="/pp-mark.png" alt="" width={8} height={8}
                  ...
                  <div style={{ padding: '7px 8px' }}>
                    <p style={{ fontFamily: 'Open Sans', fontWeight: 700, fontSize: 11, color: '#071126', margin: 0, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {r.name}
                    </p>
